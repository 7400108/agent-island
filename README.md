...
1. 打开 `release/Agent-Island-1.2.0-x64-Portable.exe`，或运行 `release/Agent-Island-1.2.0-x64-Setup.exe` 安装。
2. 点击屏幕顶部中央的胶囊，再点击右上角设置图标。
...
空闲时胶囊始终显示。点击外部或按 Esc 收起。新答案自动展示 2 秒；在此期间点击或滚动内容会转为手动阅读，不再自动关闭。手动阅读或修改设置时，新答案不会打断当前页面。托盘图标可打开通知中心、设置或退出。程序默认静音，不开机启动；设置中可开启这些选项、切换显示器、调整顶部间距、减少动画。
